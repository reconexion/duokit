...
const ytdlpArgs = require('./ytdlp-args');
const downloadTicket = require('./download-ticket');
...
// Validación, límites de calidad y armado de argumentos de yt-dlp viven en ytdlp-args.js (compartido con el
// Asistente de escritorio, helper/) — aquí solo se le pasan los trucos de bloqueo de IP (ytdlpStrategyArgs), que
// son específicos de este servidor y no le hacen falta al Asistente corriendo en la compu del cliente.
const { validatePayload, VIDEO_QUALITY_MAP, buildVideoArgs: buildVideoArgsBase, buildAudioArgs: buildAudioArgsBase, buildThumbnailArgs } = ytdlpArgs;
const buildVideoArgs = (config) => buildVideoArgsBase(config, LIMITS, ytdlpStrategyArgs());
const buildAudioArgs = (config) => buildAudioArgsBase(config, LIMITS, ytdlpStrategyArgs());
...
// Instaladores del Asistente de escritorio (helper/): pública, no hace falta sesión (es antes de poder entrar).
// Los archivos de verdad NO viven en el repo (pesan decenas de MB cada uno) — se suben al volumen persistente
// (railway volume files upload, ver deploy/RAILWAY.md) en HELPER_DIST_DIR/<archivo>. Si todavía no se subieron
...
app.get('/asistente/:os', (req, res) => {
  const filename = HELPER_FILES[req.params.os];
  if (!filename) return res.status(404).json({ error: 'Sistema operativo no reconocido.' });
  const file = path.join(HELPER_DIST_DIR, filename);
  if (!fs.existsSync(file)) return res.status(404).json({ error: 'El instalador todavía no está disponible. Escríbenos.' });
...
// Para el Asistente de escritorio (helper/): la descarga real la hace la compu del cliente, no este servidor, pero
// SI se puede o no descargar (calidad del plan, tope diario, bloqueo automático) lo sigue decidiendo este servidor,
// exactamente igual que /api/download — nada más que en vez de arrancar el trabajo aquí, firma un ticket que el
// Asistente va a verificar antes de ejecutar nada (ver download-ticket.js). Sin maxConcurrentJobs/maxJobsPerUser:
// esos protegen los recursos DE ESTE SERVIDOR, que aquí no se usan para nada.
app.post('/api/download-ticket', auth.requireAuth, (req, res) => {
  const errors = validatePayload(req.body);
  if (errors.length > 0) {
    return res.status(400).json({ error: errors.join(' ') });
  }
  ...
  const ticket = downloadTicket.issue({
    url: req.body.url.trim(),
    downloadVideo: Boolean(req.body.downloadVideo),
    downloadAudio: Boolean(req.body.downloadAudio),
    downloadThumbnail: Boolean(req.body.downloadThumbnail),
    videoQuality: req.body.videoQuality,
    audioQuality: req.body.audioQuality,
    audioLang: req.body.audioLang,
    startTime: req.body.startTime,
    endTime: req.body.endTime,
    maxDurationMin: LIMITS.maxDurationMin,
    maxFileSize: LIMITS.maxFileSize,
  });
  res.json({ ticket, ttlMs: downloadTicket.TTL_MS });
});
