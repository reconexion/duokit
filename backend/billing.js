// Activación de cuentas al confirmar un pago. Crea el usuario (o renueva el existente) y avisa
// por el notificador registrado (el bot de Telegram) sin depender de él directamente.
const auth = require('./auth');
const audit = require('./audit');
const payments = require('./payments');
const { PLANS } = require('./plans');

let notifier = null; // { deliverActivation(info) -> boolean, notifyAdmin(text) }
const setNotifier = (n) => {
  notifier = n;
};
...
// Las confirmaciones se hacen de una en una: cada una lee y escribe usuarios y pagos, y un doble clic (o dos pagos
// confirmados a la vez desde /admin) no debe pisar a la otra. La segunda espera y ve el estado ya actualizado.
...
async function activate(payment) {
  ...
  let user = auth.findUserByTelegramId(payment.telegramId);
  ...
  const username = auth.uniqueUsername(payment.telegramUsername || payment.payerName || payment.telegramName || 'duo');
  user = await auth.addUser({
    username,
    name: payment.payerName || payment.telegramName || payment.telegramUsername || username,
    password,
    plan: plan.id,
...
// Confirma un pago pendiente: activa la cuenta y le manda al cliente sus datos y el recibo.
const confirm = (reference, by = 'admin') => serialized(() => confirmNow(reference, by));

async function confirmNow(reference, by) {
  ...
  if (notifier) {
    try {
      delivered = await notifier.deliverActivation({ payment: confirmed, user, password, created, accessUntil });
    } catch (err) {
      console.error('No se pudo avisar al cliente por Telegram:', err.message);
    }
  }
  // Si Telegram falló, el administrador necesita las credenciales para dárselas por otro medio.
  return { payment: confirmed, user, created, delivered, credentials: !delivered && created ? { username: user.username, password } : null };
}
...
module.exports = { confirm, cancel, setNotifier, getNotifier };
